...
const ProgressBar = ({ status, approverRole, onClick }: { status: string; approverRole: string | null; onClick: () => void }) => {
  const getProgress = () => {
    ...
    if (status === "ACCEPTED" && !approverRole) return { width: "50%", text: "Accepted - Pending Approval" };
    if (status === "ACCEPTED" && approverRole === "HOD") return { width: "100%", text: "Approved by HOD" };
