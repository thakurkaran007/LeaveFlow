...
export const createLeave = async (lectureId: string, reason: string, weekDay: number) => {
    ...
    try {
        await db.$transaction(async (tx) => {
            ...
            if (lecture) {
                const potentialAccepters = await tx.user.findMany({
                    where: {
                        role: "TEACHER",
                        id: { not: requesterId },
                        lectures: {
                ...
                for (const accepter of potentialAccepters) {
